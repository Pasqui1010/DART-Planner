...
from src.planning.global_mission_planner import GlobalMissionPlanner, GlobalMissionConfig, SemanticWaypoint
...
def test_neural_scene_integration():
    ...
    assert global_planner.neural_scene_model is not None, "Neural scene model should be initialized"
    assert global_planner.uncertainty_field is not None, "Uncertainty field should be initialized"
    ...
    print(f"   High Uncertainty: {uncertainty_stats['high_uncertainty_percentage']:.1f}%")
    ...
    for step in range(flight_duration * update_frequency):
        ...
        if step % 5 == 0 and global_planner.neural_scene_model is not None:  # Every 2.5 seconds
            # Query density at current position (for DIAL-MPC collision detection)
            density = global_planner.neural_scene_model.query_density(drone_state.position)
            
            # Query uncertainty at current position (for exploration planning)
            uncertainty = global_planner.neural_scene_model.query_uncertainty(drone_state.position)
            
            # Query semantic label (for intelligent navigation)
            semantic_label, confidence = global_planner.neural_scene_model.query_semantic_label(drone_state.position)
        ...
        if step % 10 == 0:  # Every 5 seconds
            exploration_targets = global_planner.uncertainty_field.get_exploration_targets(
    ...
    print(f"   Observation Coverage: {final_uncertainty_stats['observation_coverage']:.1%}")
    print(f"   High Uncertainty: {final_uncertainty_stats['high_uncertainty_percentage']:.1f}%")
    print(f"   Average Uncertainty: {final_uncertainty_stats['average_uncertainty']:.3f}")
    ...
    for i, result in enumerate(batch_results[:3]):  # Show first 3
        print(f"      Query {i+1}: density={result.density:.3f}, uncertainty={result.uncertainty:.3f}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("   1. Integrate actual NeRF/3DGS models (replace PlaceholderNeuralScene)")

...
def main(duration: Optional[float] = 30.0):
    ...
    try:
        print(f"Edge node starting. Running for {duration} seconds. Press Ctrl+C to stop early.")
        ...
        while True:
            ...
            if comm_counter % comm_frequency == 0:
                ...
                if new_trajectory:
                    ...
                    print(f"Received new trajectory. Target center: {trajectory.positions[0]}")
                else:
                    print("Communication failed. Continuing with old trajectory or fallback.")
            
            control_command, target_pos = onboard_controller.compute_control_command(current_state, trajectory)
            current_state = drone_simulator.step(current_state, control_command, dt)
            
            # Log data for analysis
            log_data.append([current_state.timestamp, 
                             *current_state.position, 
                             *target_pos])
            
            if comm_counter % 100 == 0:
                 print(f"State @ {current_state.timestamp:.2f}: Pos={np.round(current_state.position, 2)}")
        ...
        with open(log_filename, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["timestamp", "actual_x", "actual_y", "actual_z", "target_x", "target_y", "target_z"])

...
def create_test_trajectory(duration=10.0, dt=0.01) -> Trajectory:
    ...
    for t in timestamps:
        if t < 2.0:  # Accelerate up
            ...
            t_decel = t - 8.0
            accel = np.array([0, 0, -1.0])
            pos = np.array([0, 0, 14.0]) + np.array([0, 0, 2.0]) * t_decel + 0.5 * accel * t_decel**2
...
def plot_results(log):
    ...
    for i, (axis, color) in enumerate(zip(['X', 'Y', 'Z'], ['r', 'g', 'b'])):
        axs[i, 0].plot(timestamps, [item['actual_pos'][i] for item in log], f'{color}-', label=f'Actual {axis}')
        axs[i, 0].plot(timestamps, [item['target_pos'][i] for item in log], f'{color}--', label=f'Target {axis}')
    ...
    # Velocity plots
    for i, (axis, color) in enumerate(zip(['X', 'Y', 'Z'], ['r', 'g', 'b'])):
        axs[i, 1].plot(timestamps, [item['actual_vel'][i] for item in log], f'{color}-', label=f'Actual {axis} Vel')
        axs[i, 1].plot(timestamps, [item['target_vel'][i] for item in log], f'{color}--', label=f'Target {axis} Vel')
...
def run_feedforward_test():
    ...
    test_config.pos_x.Kp = 0.0; test_config.pos_x.Ki = 0.0
    test_config.pos_y.Kp = 0.0; test_config.pos_y.Ki = 0.0
    test_config.pos_z.Kp = 0.0; test_config.pos_z.Ki = 0.0
    ...
    log = []
    current_state = DroneState(timestamp=0, position=np.zeros(3), velocity=np.zeros(3), attitude=np.zeros(3), angular_velocity=np.zeros(3))
    ...
    for t_np in np.arange(0, duration, dt):
        ...
        if interp_idx < len(trajectory.timestamps) and trajectory.velocities is not None:
            target_vel = trajectory.velocities[interp_idx]
            
        log.append({'timestamp': t, 'actual_pos': current_state.position, 'target_pos': target_pos, 'actual_vel': current_state.velocity, 'target_vel': target_vel})
...
def run_feedback_test():
    ...
    test_config.pos_x.Kp = 0.5; test_config.pos_x.Ki = 0.0
    test_config.pos_y.Kp = 0.5; test_config.pos_y.Ki = 0.0
    test_config.pos_z.Kp = 1.0; test_config.pos_z.Ki = 0.0
    
    print(f"Gains: Kp=[{test_config.pos_x.Kp}, {test_config.pos_y.Kp}, {test_config.pos_z.Kp}], Ki=0")
    ...
    for t_np in np.arange(0, duration, dt):
        ...
        cmd, target_pos = controller.compute_control_command(current_state, hover_trajectory)
        current_state = simulator.step(current_state, cmd, dt)
        
        log.append({'timestamp': t, 'actual_pos': current_state.position, 'target_pos': target_pos, 'actual_vel': current_state.velocity, 'target_vel': np.zeros(3)})
...
def run_integrated_test():
    ...
    print(f"Gains: Kp=[{test_config.pos_x.Kp}, {test_config.pos_y.Kp}, {test_config.pos_z.Kp}], "
...
def load_gains_from_file(filepath="src/control/control_config.py") -> ControlConfig:
    ...
    def parse_pid(name: str) -> PIDGains:
        pattern = re.compile(rf"{name}\s*=\s*PIDGains\s*\(\s*Kp=([\d.]+),\s*Ki=([\d.]+),\s*Kd=([\d.]+),\s*integral_limit=([\d.]+)\)")
...
def run_attitude_test():
    ...
    test_config.pos_x.Kp = 0.0; test_config.pos_x.Ki = 0.0; test_config.pos_x.Kd = 0.0
    test_config.pos_y.Kp = 0.0; test_config.pos_y.Ki = 0.0; test_config.pos_y.Kd = 0.0
    test_config.pos_z.Kp = 0.0; test_config.pos_z.Ki = 0.0; test_config.pos_z.Kd = 0.0

    controller = OnboardController(config=test_config)
    print(f"Controller using Attitude Gains: Kp={controller.config.roll.Kp}, Ki={controller.config.roll.Ki}, Kd={controller.config.roll.Kd}")
    ...
    for t_np in np.arange(0, duration, dt):
        ...
        torque = controller._compute_torque(target_roll, target_pitch, target_yaw_rate, current_state, dt)
...
def plot_attitude_results(log):
    ...
    # Roll plot
    axs[0].plot(timestamps, np.rad2deg([item['actual_roll'] for item in log]), 'r-', label='Actual Roll')
    axs[0].plot(timestamps, np.rad2deg([item['target_roll'] for item in log]), 'r--', label='Target Roll')
    axs[0].set_ylabel('Roll Angle (deg)')
    axs[0].legend()
    axs[0].grid(True)

    # Pitch plot
    axs[1].plot(timestamps, np.rad2deg([item['actual_pitch'] for item in log]), 'g-', label='Actual Pitch')
    axs[1].plot(timestamps, np.rad2deg([item['target_pitch'] for item in log]), 'g--', label='Target Pitch')

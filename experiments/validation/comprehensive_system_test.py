...
def analyze_recent_logs():
    ...
    try:
        ...
        print(f"  📏 Average position error: {avg_pos_error:.3f}m" if avg_pos_error != "N/A" else "  📏 Position error: N/A")
        print(f"  📏 Maximum position error: {max_pos_error:.3f}m" if max_pos_error != "N/A" else "  📏 Max position error: N/A")
        print(f"  🚀 Average velocity: {avg_velocity:.2f}m/s" if avg_velocity != "N/A" else "  🚀 Average velocity: N/A")
        print(f"  🚀 Maximum velocity: {max_velocity:.2f}m/s" if max_velocity != "N/A" else "  🚀 Maximum velocity: N/A")
...
def create_comprehensive_visualization(test_results, log_analysis):
    ...
    if test_results and log_analysis:
        ...
        ax2.text(0.5, 0.5, 'No test results available', ha='center', va='center', 
    ...
    if test_results:
        ax4.plot(test_results['times'], test_results['control_errors'], 
        ...
        ax4.annotate(f'Final Error: {final_error:.3f}m', 
    ...
    if test_results and len(test_results['planning_times']) > 0:
        ax5.hist(test_results['planning_times'], bins=20, alpha=0.7, color='green', edgecolor='black')
        ax5.axvline(np.mean(test_results['planning_times']), color='red', linestyle='--', 
                   linewidth=2, label=f'Mean: {np.mean(test_results["planning_times"]):.2f}ms')
        ax5.set_xlabel('Planning Time (ms)')
        ax5.set_ylabel('Frequency')
        ax5.set_title('DIAL-MPC Planning Time Distribution', fontsize=14, fontweight='bold')
        ax5.legend()
        ax5.grid(True, alpha=0.3)
    else:
        ax5.text(0.5, 0.5, 'No planning time data available', ha='center', va='center',
                transform=ax5.transAxes, fontsize=12)
        ax5.set_title('Planning Time Distribution (No Data)', fontsize=14, fontweight='bold')
    ...
    if log_analysis and log_analysis['avg_pos_error'] != "N/A":
        ...
        if 'pos_x' in df.columns:
            ...
            ax6.axvline(pos_errors.mean(), color='red', linestyle='--', 
                       linewidth=2, label=f'Mean: {pos_errors.mean():.3f}m')
            ax6.set_xlabel('Position Error (m)')
            ax6.set_ylabel('Frequency')
            ax6.set_title('Position Error Distribution (Log Data)', fontsize=14, fontweight='bold')
            ax6.legend()
            ax6.grid(True, alpha=0.3)
        else:
            ax6.text(0.5, 0.5, 'No position error data in log', ha='center', va='center',
                    transform=ax6.transAxes, fontsize=12)
            ax6.set_title('Position Error Distribution (No Data)', fontsize=14, fontweight='bold')
    else:
        ax6.text(0.5, 0.5, 'No log data available', ha='center', va='center',
                transform=ax6.transAxes, fontsize=12)
        ax6.set_title('Position Error Distribution (No Log)', fontsize=14, fontweight='bold')
    ...
    if test_results:
        ...
        ax7.plot(positions[:, 0], positions[:, 1], positions[:, 2], 
                linewidth=2, alpha=0.8, label='Actual Trajectory')
        ...
        ax7.scatter(10.0, 5.0, -15.0, color='orange', s=150, marker='*', label='Target', alpha=0.8)
        ...
        ax7.text(0.5, 0.5, 0.5, 'No trajectory data available', ha='center', va='center',
    ...
    ax8.text(0.05, 0.95, status_text, transform=ax8.transAxes, fontsize=12,
            verticalalignment='top', fontfamily='monospace',
            bbox=dict(boxstyle='round', facecolor='lightgray', alpha=0.8))
    
    # Add title and metadata
    fig.suptitle('Comprehensive System Test & Performance Analysis', 
...
def main():
    ...
    print(f"  🔧 Component Status: {'✅ All components functional' if all_components_ok else '❌ Some components failed'}")

...
class DIALMPCPlanner:
    ...
    def plan_trajectory(
        ...
        if self.last_solution is not None:
            ...
            expected_pos = (
                self.last_solution[1] if len(self.last_solution) > 1 else current_pos
            )
        ...
        try:
            ...
            if len(positions) > 1:
                ...
                if first_step_distance > max_reasonable_step:
                    print(
                        f"WARNING: Generated trajectory too aggressive: {first_step_distance:.1f}m first step"
                    )
    ...
    def _solve_dial_mpc_consistent(
        self, pos0: np.ndarray, vel0: np.ndarray, acc0: np.ndarray
    ...
    def _improved_warm_start_solution(
        self, current_pos: np.ndarray, current_vel: np.ndarray
    ) -> np.ndarray:
        ...
        if self.last_solution is not None and len(self.last_solution) > 2:
            ...
            for i in range(1, shift_length + 1):
                if i < len(self.last_solution):
                    ...
                    positions[i] = self.last_solution[i] + position_correction * (
                        1.0 - i / shift_length
                    )
    ...
    def _initialize_smooth_trajectory(
        self, pos0: np.ndarray, vel0: np.ndarray, N: int
    ) -> np.ndarray:
        ...
        if self.goal_position is not None:
            ...
            for i in range(1, N + 1):
                ...
                positions[i] = (
                    pos0 + s * goal_vec + vel0 * (i * self.config.dt) * vel_influence
                )
        else:
    ...
    def _compute_stable_cost_and_gradient(
        self,
        ...
        initial_cost = 100.0 * np.sum(
            initial_vel_error**2
        )  # Strong initial condition constraint
        ...
        for i in range(N + 1):
            ...
            if self.goal_position is not None:
                ...
                total_cost += cost_pos
                gradient[i] += (
                    2 * self.config.position_weight * progress_weight * pos_error
                )
            ...
            if self.last_solution is not None and i < len(self.last_solution):
                ...
                stability_cost = 1.0 * np.sum(
                    stability_error**2
                )  # Soft constraint on changes
    ...
    def _compute_adaptive_step_size(
        self, iteration: int, gradient: np.ndarray
    ) -> float:
    ...
    def _generate_conservative_trajectory(
        ...
        if self.goal_position is not None:
            ...
            if goal_distance > 0.1:
                ...
                max_conservative_vel = min(
                    self.config.max_velocity * 0.5, float(goal_distance / (N * dt))
                )
                ...
                for i in range(1, N + 1):
                    ...
                    velocities[i] = (
                        1 - alpha
                    ) * current_state.velocity + alpha * target_velocity
        ...
        for i in range(N + 1):
            ...
            for obs_center, obs_radius in self.obstacles:
                ...
                if dist < safety_dist:
                    ...
                    penalty = self.config.obstacle_weight * np.exp(
                        -(dist - safety_dist)
                    )
                    total_cost += penalty
    ...
    def get_planning_stats(self) -> dict:
        ...
        return {
            ...
            "recent_avg_ms": np.mean(self.planning_times[-10:]) * 1000
            if len(self.planning_times) >= 10
            else 0,
        }

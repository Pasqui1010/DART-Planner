...
def create_status_visualization(component_results, log_data):
    ...
    if log_data:
        ...
        if 'actual_x' in df.columns and 'desired_x' in df.columns:
            ...
            ax2.text(0.5, 0.5, 'No position tracking data', ha='center', va='center',
                    transform=ax2.transAxes)
            ax2.set_title('Position Tracking (No Data)', fontweight='bold')
    else:
        ax2.text(0.5, 0.5, 'No log data available', ha='center', va='center',
                transform=ax2.transAxes)
    ...
    if log_data and 'actual_vx' in log_data['dataframe'].columns:
        df = log_data['dataframe']
        time_col = log_data.get('time_column', 'time')
        velocities = np.sqrt(df['actual_vx']**2 + df['actual_vy']**2 + df['actual_vz']**2)
        ...
        ax3.text(0.02, 0.98, f'Mean: {mean_vel:.2f}m/s\nMax: {max_vel:.2f}m/s', 
                transform=ax3.transAxes, va='top',
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    else:
        ax3.text(0.5, 0.5, 'No velocity data available', ha='center', va='center',
...
def main():
    ...
    print(f"\n📋 FINAL SUMMARY:")
    print(f"  🔧 Components: {'✅ All functional' if all_components_ok else '❌ Issues detected'}")
    print(f"  📊 Data Analysis: {'✅ Recent logs found' if log_data else '❌ No recent data'}")

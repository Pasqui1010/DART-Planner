...
class HardwareConfig(BaseModel):
    ...
    enable_timing_alignment: bool = Field(default=True, description="Enable timing alignment between planner and controller")
    max_planning_latency: float = Field(default=0.1, ge=0.01, le=1.0, description="Maximum planning latency before throttling")
    min_planning_interval: float = Field(default=0.01, ge=0.001, le=0.1, description="Minimum interval between planning calls")
    enable_controller_throttling: bool = Field(default=True, description="Enable controller throttling when planner is slow")
    enable_trajectory_interpolation: bool = Field(default=True, description="Enable trajectory interpolation for smooth control")
...
class DARTPlannerConfig(BaseModel):
    ...
    def validate_secret_key(cls, v):
        ...
        if v.enable_authentication and not v.secret_key and os.getenv('DART_ENVIRONMENT') == 'production':
            ...
            raise ConfigurationError("Secret key must be set when authentication is enabled in production")
        return v
    ...
    def validate_environment(cls, v):
        ...
        if v not in ['development', 'production', 'testing']:
            from dart_planner.common.errors import ConfigurationError
            raise ConfigurationError("Environment must be one of: development, production, testing")
        return v
...
class ConfigManager:
    ...
    def _load_from_env(self, config_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        env_mappings = {
            ...
            'DART_ENABLE_TIMING_ALIGNMENT': 'hardware.enable_timing_alignment',
            'DART_MAX_PLANNING_LATENCY': 'hardware.max_planning_latency',
            'DART_MIN_PLANNING_INTERVAL': 'hardware.min_planning_interval',
            'DART_ENABLE_CONTROLLER_THROTTLING': 'hardware.enable_controller_throttling',
            'DART_ENABLE_TRAJECTORY_INTERPOLATION': 'hardware.enable_trajectory_interpolation',
        ...
        for env_var, config_path in env_mappings.items():
            ...
            if env_value is not None:
                ...
                if env_var in ['DART_DEBUG', 'DART_ENABLE_HEARTBEAT', 'DART_ENABLE_AUTHENTICATION',
                             'DART_ENABLE_TIMING_ALIGNMENT', 'DART_ENABLE_CONTROLLER_THROTTLING',
                             'DART_ENABLE_TRAJECTORY_INTERPOLATION']:
                    ...
                    env_value = int(env_value)
                               ...
                               'DART_MAX_PLANNING_LATENCY', 'DART_MIN_PLANNING_INTERVAL']:
...
def get_safety_limits() -> Dict[str, Any]:
    """Get safety limits configuration (backward compatibility)."""
    config = get_config()
    return config.get_safety_config_dict()

def get_controller_params(name: str | None = None) -> Dict[str, Any]:
    """Get controller parameters (backward compatibility)."""
    config = get_config()
    hardware_config = config.get_hardware_config_dict()
    if name is None:

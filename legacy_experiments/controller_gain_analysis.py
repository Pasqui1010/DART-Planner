...
class ControllerGainAnalysis:
    ...
    def analyze_current_configuration(self):
        ...
        print(f"   Legacy Config - Kp: [{self.legacy_config.pos_x.Kp}, {self.legacy_config.pos_y.Kp}, {self.legacy_config.pos_z.Kp}]")
        print(f"   Legacy Config - Ki: [{self.legacy_config.pos_x.Ki}, {self.legacy_config.pos_y.Ki}, {self.legacy_config.pos_z.Ki}]")
        print(f"   Legacy Config - Kd: [{self.legacy_config.pos_x.Kd}, {self.legacy_config.pos_y.Kd}, {self.legacy_config.pos_z.Kd}]")
        
        print(f"\n🎯 Attitude Control Gains:")
        print(f"   Geometric Config - Kp: {self.geometric_config.kp_att}")
        print(f"   Geometric Config - Kd: {self.geometric_config.kd_att}")
        print(f"   Legacy Config - Kp: [{self.legacy_config.roll.Kp}, {self.legacy_config.pitch.Kp}, {self.legacy_config.yaw_rate.Kp}]")
        print(f"   Legacy Config - Kd: [{self.legacy_config.roll.Kd}, {self.legacy_config.pitch.Kd}, {self.legacy_config.yaw_rate.Kd}]")
    ...
    def _analyze_gain_ratios(self):
        ...
        for axis, label in enumerate(['X', 'Y', 'Z']):
            ...
            print(f"   {label}-axis: Kp={kp:.1f}, Ki/Kp={ki_kp_ratio:.2f}, Kd/Kp={kd_kp_ratio:.2f}")
            
            # Check for common tuning issues
            if ki_kp_ratio > 0.3:
                print(f"      ⚠️  High integral gain ratio - may cause instability")
            if kd_kp_ratio > 1.0:
                print(f"      ⚠️  High derivative gain ratio - may cause noise sensitivity")
            if kp < 1.0:

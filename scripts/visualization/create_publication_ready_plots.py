...
def create_publication_plots():
    ...
    position_error = np.sqrt(
        (data["actual_x"] - data["desired_x"]) ** 2
    ...
    actual_speed = np.sqrt(
        data["actual_vx"] ** 2 + data["actual_vy"] ** 2 + data["actual_vz"] ** 2
    )
    desired_speed = np.sqrt(
        data["desired_vx"] ** 2 + data["desired_vy"] ** 2 + data["desired_vz"] ** 2
    )

    ax4.plot(data["timestamp"], actual_speed, "b-", linewidth=2, label="Actual Speed")
    ax4.plot(
        data["timestamp"], desired_speed, "r--", linewidth=2, label="Desired Speed"
    )
    ...
    line1 = ax5.plot(
        data["timestamp"], data["thrust"], "b-", linewidth=2, label="Thrust"
    )
    line2 = ax5_twin.plot(
        data["timestamp"], data["torque_norm"], "r-", linewidth=2, label="Torque"
    )
...
def create_standalone_3d_plot(data):
    ...
    ax.set_title(
        "Distributed Control System: 3D Trajectory Tracking\nAdvanced Performance Metrics",
        fontsize=16,
        pad=20,
    )

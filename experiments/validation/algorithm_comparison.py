...
def run_algorithm_comparison():
    ...
    for scenario in test_scenarios:
        ...
        for alg_name, planner in [('DIAL-MPC', dial_mpc), ('SE(3)-MPC', se3_mpc)]:
            ...
            try:
                while time.time() - start_time < test_duration:
                    ...
                    if scenario == 'hover':
                        ...
                        goal_pos = np.array([5.0, 0.0, 3.0]) if current_time > 2.0 else np.array([0.0, 0.0, 2.0])
                    ...
                    try:
                        ...
                        if len(trajectory.positions) > 0:
                            ...
                            desired_vel = trajectory.velocities[0] if trajectory.velocities is not None else np.zeros(3)
                            desired_acc = trajectory.accelerations[0] if trajectory.accelerations is not None else np.zeros(3)
                        else:
                    ...
                    try:
                        control_output = controller.compute_control(state, desired_pos, desired_vel, desired_acc)
            ...
            result = ComparisonResults(
                ...
                max_position_error=float(np.max(position_errors)) if position_errors else float('inf'),
                mean_planning_time_ms=float(np.mean(planning_times)) if planning_times else float('inf'),
                success_rate=float(np.mean(successes)) if successes else 0.0,
                stability_metric=float(np.std(position_errors)) if len(position_errors) > 1 else float('inf')
            )
    ...
    for scenario, alg_results in by_scenario.items():
        if 'DIAL-MPC' in alg_results and 'SE(3)-MPC' in alg_results:
            ...
            if se3_error < dial_error:
                ...
                print(f"{scenario:15s}: SE(3)-MPC better by {improvement:.1f}% ({dial_error:.2f}m → {se3_error:.2f}m)")
                se3_better += 1
            else:
                degradation = (se3_error - dial_error) / dial_error * 100
                print(f"{scenario:15s}: DIAL-MPC better by {degradation:.1f}% ({se3_error:.2f}m → {dial_error:.2f}m)")
                dial_better += 1
    ...
    for scenario, alg_results in by_scenario.items():
        if 'DIAL-MPC' in alg_results and 'SE(3)-MPC' in alg_results:
            ...
            if se3_time < dial_time:
                ...
                print(f"{scenario:15s}: SE(3)-MPC {speedup:.1f}x faster ({dial_time:.1f}ms → {se3_time:.1f}ms)")
            else:
                slowdown = se3_time / dial_time
                print(f"{scenario:15s}: DIAL-MPC {slowdown:.1f}x faster ({se3_time:.1f}ms → {dial_time:.1f}ms)")
    ...
    if se3_better > dial_better:
        print(f"✅ SE(3) MPC outperforms DIAL-MPC in {se3_better}/{se3_better + dial_better} scenarios")
        print("   REFACTOR FROM DIAL-MPC TO SE(3) MPC IS VALIDATED")
        print("   Technical audit finding confirmed: DIAL-MPC is mismatched for aerial robotics")
        ...
        print(f"❌ DIAL-MPC outperforms SE(3) MPC in {dial_better}/{se3_better + dial_better} scenarios")

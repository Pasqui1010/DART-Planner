...
def test_controller_performance(controller, trajectory, simulator):
    ...
    metrics = {
        ...
        'rms_position_error': np.sqrt(np.mean(np.array(results['position_errors'])**2)),
...
def create_comparison_plot(results1, metrics1, results2, metrics2, labels):
    ...
    fig.suptitle('Optimized vs Original Controller Performance Comparison', fontsize=16, fontweight='bold')
    
    # 1. 3D Trajectory Plot
    ax = fig.add_subplot(2, 3, 1, projection='3d')
    ax.plot(results1['desired_pos'][:, 0], results1['desired_pos'][:, 1], results1['desired_pos'][:, 2], 
            'k--', linewidth=2, label='Desired', alpha=0.8)
    ax.plot(results1['actual_pos'][:, 0], results1['actual_pos'][:, 1], results1['actual_pos'][:, 2], 
            'b-', linewidth=1.5, label=labels[0], alpha=0.8)
    ax.plot(results2['actual_pos'][:, 0], results2['actual_pos'][:, 1], results2['actual_pos'][:, 2], 
    ...
    ax.plot(results1['time'], results1['position_errors'], 'b-', label=labels[0], linewidth=1.5)
    ax.plot(results2['time'], results2['position_errors'], 'r-', label=labels[1], linewidth=1.5)
    ...
    ax.plot(results1['time'], results1['velocity_errors'], 'b-', label=labels[0], linewidth=1.5)
    ax.plot(results2['time'], results2['velocity_errors'], 'r-', label=labels[1], linewidth=1.5)
    ax.set_xlabel('Time (s)')
    ...
    metrics_names = ['Mean Pos Error', 'Max Pos Error', 'RMS Pos Error', 'Mean Vel Error']
    metrics1_values = [metrics1['mean_position_error'], metrics1['max_position_error'], 
                      metrics1['rms_position_error'], metrics1['mean_velocity_error']]
    metrics2_values = [metrics2['mean_position_error'], metrics2['max_position_error'], 
                      metrics2['rms_position_error'], metrics2['mean_velocity_error']]
    
    x = np.arange(len(metrics_names))
    width = 0.35
    
    bars1 = ax.bar(x - width/2, metrics1_values, width, label=labels[0], color='blue', alpha=0.7)
    bars2 = ax.bar(x + width/2, metrics2_values, width, label=labels[1], color='red', alpha=0.7)
    ...
    ax.plot(results1['time'], results1['thrust_commands'], 'b-', label=f'{labels[0]} Thrust', alpha=0.8)
    ax.plot(results2['time'], results2['thrust_commands'], 'r-', label=f'{labels[1]} Thrust', alpha=0.8)
    ax.set_xlabel('Time (s)')
    ...
    for bar, val in zip(bars, improvements):
        ax.text(bar.get_x() + bar.get_width()/2., val + (1 if val > 0 else -1),
...
def main():
    ...
    pos_improvement = ((metrics_original['mean_position_error'] - metrics_optimized['mean_position_error']) 
                      / metrics_original['mean_position_error']) * 100
    max_improvement = ((metrics_original['max_position_error'] - metrics_optimized['max_position_error']) 
                      / metrics_original['max_position_error']) * 100
    vel_improvement = ((metrics_original['mean_velocity_error'] - metrics_optimized['mean_velocity_error']) 
                      / metrics_original['mean_velocity_error']) * 100
    ...
    fig = create_comparison_plot(
        results_optimized, metrics_optimized,
        results_original, metrics_original,
        ['Optimized', 'Original']
    )

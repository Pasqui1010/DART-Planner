...
def analyze_phase2a_results():
    ...
    try:
        ...
        if mean_vel_2a < target_vel_error:
            ...
            print(f"   ⚠️  PARTIAL SUCCESS: {mean_vel_2a:.2f}m/s > {target_vel_error}m/s target")
        ...
        for i, label in enumerate(percentile_labels):
            change = ((vel_percentiles_2a[i] - vel_percentiles_1[i]) / vel_percentiles_1[i]) * 100
            print(f"   {label} percentile: {vel_percentiles_1[i]:.2f} → {vel_percentiles_2a[i]:.2f}m/s ({change:+.1f}%)")
        ...
        print(f"   Phase 2A (Derivative Gains): {'✅ COMPLETE' if mean_vel_2a < 5.0 else '🔄 IN PROGRESS'}")
        ...
        print(f"   • Derivative gains [6,6,8]→[10,10,12]: {vel_improvement:+.1f}% velocity impact")

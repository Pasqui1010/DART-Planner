...
class TrajectorySmoother:
    ...
    def update_trajectory(self, new_trajectory: Trajectory, current_state: DroneState):
        ...
        current_des_pos, current_des_vel, current_des_acc = self._interpolate_trajectory(
        ...
        if pos_diff > 0.5 or vel_diff > 1.0:
            ...
            print(f"Starting smooth transition: pos_diff={pos_diff:.2f}, vel_diff={vel_diff:.2f}")
        
        # Update trajectory
        self.current_trajectory = new_trajectory
        self.trajectory_start_time = current_time
    
    def get_desired_state(self, current_time: float, current_state: DroneState) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        if self.in_transition:
            transition_progress = (current_time - self.transition_start_time) / self.transition_time
        ...
        if self.current_trajectory is not None:
            return self._interpolate_trajectory(current_time, self.current_trajectory, self.trajectory_start_time)
        else:
            # No trajectory available - hover
            return current_state.position, np.zeros(3), np.zeros(3)
    
    def _interpolate_trajectory(self, current_time: float, trajectory: Trajectory, start_time: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if trajectory_time <= traj_times[0]:
            ...
            vel = trajectory.velocities[0] if trajectory.velocities is not None else np.zeros(3)
            acc = trajectory.accelerations[0] if trajectory.accelerations is not None else np.zeros(3)
        elif trajectory_time >= traj_times[-1]:
            idx = len(traj_times) - 1
            pos = trajectory.positions[-1]
            vel = trajectory.velocities[-1] if trajectory.velocities is not None else np.zeros(3)
            acc = trajectory.accelerations[-1] if trajectory.accelerations is not None else np.zeros(3)
        else:
            ...
            pos = (1 - alpha) * trajectory.positions[idx] + alpha * trajectory.positions[idx + 1]
            
            if trajectory.velocities is not None:
                vel = (1 - alpha) * trajectory.velocities[idx] + alpha * trajectory.velocities[idx + 1]
            else:
                vel = np.zeros(3)
            
            if trajectory.accelerations is not None:
                acc = (1 - alpha) * trajectory.accelerations[idx] + alpha * trajectory.accelerations[idx + 1]
            else:
                acc = np.zeros(3)
        
        return pos, vel, acc
    
    def _generate_transition_state(self, progress: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Generate smooth transition using minimum jerk trajectory."""
        ...
        s_dot = (30 * t**2 - 60 * t**3 + 30 * t**4) / self.transition_time  # Velocity blend factor
        s_ddot = (60 * t - 180 * t**2 + 120 * t**3) / (self.transition_time**2)  # Acceleration blend factor
        ...
        vel = (1 - s) * self.transition_start_vel + s * self.transition_target_vel + s_dot * pos_diff
    ...
    def _get_failsafe_trajectory(self, current_time: float, current_state: DroneState) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        decay_rate = 2.0  # 1/s
        vel_decay = np.exp(-decay_rate * min(current_time - self.last_cloud_update - 2.0, 5.0))
    ...
    def is_trajectory_valid(self) -> bool:
        ...
        return (self.current_trajectory is not None and 
                time.time() - self.last_cloud_update < 2.0)

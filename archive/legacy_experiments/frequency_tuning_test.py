...
class FrequencyTuningSystem:
    ...
    def test_control_frequency(
        self, target_freq: int, test_duration: float = 20.0
    ) -> Dict[str, Any]:
        ...
        for freq in test_frequencies:
            try:
                ...
                if result.get("failsafe_rate", 0) > 50:
                    print(
                        f"⚠️  High instability at {freq}Hz - skipping higher frequencies"
                    )
                    break
    ...
    def analyze_frequency_sweep(self) -> Tuple[int, Dict[str, Any]]:
        ...
        for freq, result in self.frequency_results.items():
            ...
            print(
                f"{freq:8d}Hz | {pos_err:8.2f}m | {vel_err:8.2f}m/s | {failsafe_rate:7.1f}% | {status:6s} | {score:5.1f}"
            )
        ...
        if optimal_freq is not None:
            ...
            print(
                f"   Stability: {'✅ Stable' if optimal_result['stable'] else '❌ Unstable'}"
            )
            ...
            if optimal_result["mean_velocity_error"] < 5.0:
                ...
                print(
                    f"📊 Velocity error: {5.0 - optimal_result['mean_velocity_error']:.2f}m/s from target"
                )
        else:
    ...
    def generate_frequency_plots(self):
        ...
        if scores:
            ...
            ax4.axvline(
                ...
                linestyle="--",
                linewidth=2,
                label=f"Optimal: {optimal_freq}Hz",
            )
            ax4.legend()
...
def main():
    ...
    if optimal_result and not optimal_result.get("failed", False):
        if optimal_result["mean_velocity_error"] < 5.0:
            ...
            print(
                f"📊 Best velocity error: {optimal_result['mean_velocity_error']:.2f}m/s"
            )

...
class AirframeConfigManager:
    ...
    def _load_configs(self):
        ...
        if not self.config_path.exists():
            raise FileNotFoundError(f"Airframe config file not found: {self.config_path}")
        ...
        for name, config_data in data.items():
            ...
            if 'extends' in config_data:
                ...
                if base_name not in self._configs:
                    raise ValueError(f"Base config '{base_name}' not found for '{name}'")
    ...
    def get_config(self, name: str) -> AirframeConfig:
        ...
        if name not in self._configs:
            raise ValueError(f"Airframe config '{name}' not found. Available: {list(self._configs.keys())}")

...
def apply_phase1_optimizations():
    ...
    optimizations = {
        ...
        "kp_pos: np.ndarray = field(default_factory=lambda: np.array([6.0, 6.0, 8.0]))": "kp_pos: np.ndarray = field(default_factory=lambda: np.array([10.0, 10.0, 12.0]))",
        # Integral gains: Reduce Ki from [1,1,2] to [0.5,0.5,1] (50% reduction)
        "ki_pos: np.ndarray = field(default_factory=lambda: np.array([1.0, 1.0, 2.0]))": "ki_pos: np.ndarray = field(default_factory=lambda: np.array([0.5, 0.5, 1.0]))",
        # Derivative gains: Increase Kd from [4,4,5] to [6,6,8] (50% increase)
        "kd_pos: np.ndarray = field(default_factory=lambda: np.array([4.0, 4.0, 5.0]))": "kd_pos: np.ndarray = field(default_factory=lambda: np.array([6.0, 6.0, 8.0]))",
        # Attitude gains: Increase Kp from [10,10,4] to [12,12,5] (20% increase)
        "kp_att: np.ndarray = field(default_factory=lambda: np.array([10.0, 10.0, 4.0]))": "kp_att: np.ndarray = field(default_factory=lambda: np.array([12.0, 12.0, 5.0]))",
        # Feedforward: Increase ff_pos from 0.8 to 1.2 (50% increase)
        "ff_pos: float = 0.8": "ff_pos: float = 1.2",
        # Update the controller description
        "Optimized high-frequency geometric controller for quadrotor.": "PHASE 1 OPTIMIZED geometric controller for quadrotor.",
    }
    ...
    for old_text, new_text in optimizations.items():
        if old_text in modified_content:
            ...
            applied_optimizations.append(
                old_text.split(":")[0] if ":" in old_text else old_text[:50]
            )

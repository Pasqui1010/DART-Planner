...
def create_clean_trajectory_viz(log_file):
    ...
    # Plot trajectories
    actual_line = ax1.plot(data['actual_x'], data['actual_y'], data['actual_z'], 
                          label='Actual Trajectory', color='#2E86C1', linewidth=3, alpha=0.8)
    desired_line = ax1.plot(data[target_x], data[target_y], data[target_z], 
                           label='Desired Trajectory', color='#E74C3C', 
                           linestyle='--', linewidth=2, alpha=0.9)
    
    # Mark start and end points
    ax1.scatter([data['actual_x'].iloc[0]], [data['actual_y'].iloc[0]], 
               [data['actual_z'].iloc[0]], color='#27AE60', s=150, 
               label='Start Position', marker='o', edgecolors='black', linewidth=2)
    ax1.scatter([data['actual_x'].iloc[-1]], [data['actual_y'].iloc[-1]], 
               [data['actual_z'].iloc[-1]], color='#8E44AD', s=150, 
               label='End Position', marker='s', edgecolors='black', linewidth=2)
    ...
    ax1.set_title('Drone Trajectory Tracking Performance\nImproved Distributed Control System', 
    ...
    ax2.plot(time_rel, total_error, color='#E74C3C', linewidth=2, label='Position Error')
    ...
    ax3.axvline(np.mean(freq_clean), color='#E74C3C', linestyle='--', linewidth=2,
               label=f'Mean: {np.mean(freq_clean):.0f} Hz')
    ax3.axvline(1000, color='#27AE60', linestyle='--', linewidth=2, label='Target: 1000 Hz')
...
if __name__ == "__main__":
    ...
    if improved_files:
        latest_file = max(improved_files, key=lambda x: x.split('_')[-1].replace('.csv', ''))

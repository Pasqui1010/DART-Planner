...
def create_comprehensive_visualization(improved_log_file, original_log_files=None):
    ...
    print(
        f"Log format: {'Enhanced' if has_velocity else 'Basic'} - Columns: {list(improved_data.columns)}"
    )
    ...
    ax1.plot(
        improved_data["actual_x"],
        improved_data["actual_y"],
        improved_data["actual_z"],
        label="Actual Trajectory",
        color="blue",
        linewidth=2,
    )
    ax1.plot(
        improved_data[target_x],
        improved_data[target_y],
        improved_data[target_z],
        label="Target/Desired Trajectory",
        color="red",
        linestyle="--",
        linewidth=2,
    )
    ...
    ax1.scatter(
        [improved_data["actual_x"].iloc[0]],
        [improved_data["actual_y"].iloc[0]],
        [improved_data["actual_z"].iloc[0]],
        color="green",
        s=100,
        label="Start",
    )
    ax1.scatter(
        [improved_data["actual_x"].iloc[-1]],
        [improved_data["actual_y"].iloc[-1]],
        [improved_data["actual_z"].iloc[-1]],
        color="purple",
        s=100,
        label="End",
    )
    ...
    if has_velocity:
        ...
        if "desired_vx" in improved_data.columns:
            desired_vel_mag = np.sqrt(
                improved_data["desired_vx"] ** 2
    ...
    ax4.plot(
        time_rel, improved_data[target_z], label="Target Z", color="red", linestyle="--"
    )
    ...
    error_stats = {
        "Mean Error": [
            np.mean(np.abs(pos_error_x)),
            np.mean(np.abs(pos_error_y)),
            np.mean(np.abs(pos_error_z)),
            np.mean(total_pos_error),
        ],
        "Max Error": [
            np.max(np.abs(pos_error_x)),
            np.max(np.abs(pos_error_y)),
            np.max(np.abs(pos_error_z)),
            np.max(total_pos_error),
        ],
        "RMS Error": [
            np.sqrt(np.mean(pos_error_x**2)),
            np.sqrt(np.mean(pos_error_y**2)),
            np.sqrt(np.mean(pos_error_z**2)),
            np.sqrt(np.mean(total_pos_error**2)),
        ],
    }
    ...
    ax5.bar(
        x_pos - width, error_stats["Mean Error"], width, label="Mean Error", alpha=0.8
    )
    ax5.bar(x_pos, error_stats["Max Error"], width, label="Max Error", alpha=0.8)
    ax5.bar(
        x_pos + width, error_stats["RMS Error"], width, label="RMS Error", alpha=0.8
    )
    ...
    ax6.axvline(
        1000, color="green", linestyle="--", linewidth=2, label="Target: 1000 Hz"
    )
    ...
    if has_control and rows > 2:
        ...
        ax7.plot(
            time_rel, improved_data["thrust"], label="Thrust", color="blue", linewidth=2
        )
        ax7_twin = ax7.twinx()
        ax7_twin.plot(
            time_rel,
            improved_data["torque_norm"],
            label="Torque Norm",
            color="red",
            linewidth=2,
        )
        ...
        if "failsafe_active" in improved_data.columns:
            ...
            ax9.text(
                0.02,
                0.98,
                f"Failsafe activations: {failsafe_count} ({failsafe_pct:.1f}%)",
    ...
    print(
        f"   • Actual frequency range: {np.min(control_freq):.1f} - {np.max(control_freq):.1f} Hz"
    )
    ...
    if has_velocity:
        ...
        if "desired_vx" in improved_data.columns:
            ...
            print(
                f"   • RMS velocity error: {np.sqrt(np.mean(total_vel_error**2)):.4f} m/s"
            )
    ...
    if has_control:
        ...
        print(
            f"   • Thrust range: {np.min(improved_data['thrust']):.2f} - {np.max(improved_data['thrust']):.2f} N"
        )
        print(
            f"   • Average torque norm: {np.mean(improved_data['torque_norm']):.4f} N⋅m"
        )
        ...
        if "failsafe_active" in improved_data.columns:
            ...
            print(f"\n🛡️ Safety System:")
            print(
                f"   • Failsafe activations: {failsafe_count} times ({failsafe_pct:.2f}%)"
            )
            print(
                f"   • System stability: {'EXCELLENT' if failsafe_count == 0 else 'NEEDS ATTENTION'}"
            )
    ...
    if original_log_files:
        ...
        try:
            ...
            orig_target_x = (
                "target_x" if "target_x" in original_data.columns else "desired_x"
            )
            orig_target_y = (
                "target_y" if "target_y" in original_data.columns else "desired_y"
            )
            orig_target_z = (
                "target_z" if "target_z" in original_data.columns else "desired_z"
            )
            ...
            orig_total_pos_error = np.sqrt(
                orig_pos_error_x**2 + orig_pos_error_y**2 + orig_pos_error_z**2
            )

            improvement_factor = np.mean(orig_total_pos_error) / np.mean(
                total_pos_error
            )

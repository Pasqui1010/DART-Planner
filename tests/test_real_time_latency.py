...
# Restored necessary imports with flake8 ignore for unused
from dart_planner.common.types import DroneState, Trajectory
from dart_planner.planning.se3_mpc_planner import SE3MPCPlanner, SE3MPCConfig  # noqa: F401
from dart_planner.control.geometric_controller import GeometricController  # noqa: F401
from dart_planner.common.units import Q_  # noqa: F401
...
class RealTimeLatencyTester:
    ...
    def __init__(
        ...
        self.planning_p95_threshold = get_latency_threshold(
            "PLANNING_P95_THRESHOLD", 50.0
        )  # 95th percentile planning latency
        self.control_p95_threshold = get_latency_threshold(
            "CONTROL_P95_THRESHOLD", 5.0
        )  # 95th percentile control latency
        self.actuator_p95_threshold = get_latency_threshold(
            "ACTUATOR_P95_THRESHOLD", 2.0
        )  # 95th percentile actuator latency
        self.total_p95_threshold = get_latency_threshold(
            "TOTAL_P95_THRESHOLD", 50.0
        ...
        self.test_state = DroneState(
            ...
            velocity=Q_(np.array([0.0, 0.0, 0.0]), 'm/s'),
            attitude=Q_(np.array([1.0, 0.0, 0.0, 0.0]), 'rad'),
    ...
    async def _simulate_planning_phase(
        self, current_state: DroneState, goal: np.ndarray
    ) -> Trajectory:
        """Simulate planning phase with latency measurement"""
        async with self._measure_latency("planning"):
            # Wrap raw goal array in Quantity for unit safety
            return self.planner.plan_trajectory(current_state, Q_(goal, 'm'))

    async def _simulate_control_phase(
    ...
    async def run_latency_test(self) -> LatencyTestResults:
        ...
        print("🚀 Starting Real-time Latency Test")
        ...
        while time.time() - start_time < self.test_duration:
            ...
            try:
                ...
                # Phase 2: Control
                control_cmd = await self._simulate_control_phase(
                    self.test_state, trajectory
                )
                ...
                measurement = LatencyMeasurement(
                    timestamp=time.time(),
                    planning_latency_ms=self._current_measurement.get(
                        "planning_latency_ms", 0.0
                    ),
                    control_latency_ms=self._current_measurement.get(
                        "control_latency_ms", 0.0
                    ),
                    actuator_latency_ms=self._current_measurement.get(
                ...
                if iteration % 50 == 0:
                    ...
                    print(
                        f"   Progress: {progress:.1f}% - Total: {total_latency_ms:.1f}ms"
                    )
    ...
    def print_results(self, results: LatencyTestResults):
        ...
        print("\n⚡ Latency Statistics (ms):")
        print(
            f"   {'Component':<12} {'Mean':<8} {'P50':<8} {'P95':<8} {'P99':<8} {'Max':<8} {'Status':<8}"
        )
        ...
        print(
            f"   {'Planning':<12} {results.planning_mean_ms:<8.1f} {results.planning_p50_ms:<8.1f} "
        ...
        print(
            f"   {'Control':<12} {results.control_mean_ms:<8.1f} {results.control_p50_ms:<8.1f} "
        ...
        print(
            f"   {'Actuator':<12} {results.actuator_mean_ms:<8.1f} {results.actuator_p50_ms:<8.1f} "
        ...
        print(
            f"   {'TOTAL':<12} {results.total_mean_ms:<8.1f} {results.total_p50_ms:<8.1f} "
            f"{results.total_p95_ms:<8.1f} {results.total_p99_ms:<8.1f} {results.total_max_ms:<8.1f} {status:<8}"
        )

        print("\n🎯 Performance Requirements:")
        print(
            f"   Planning P95: {results.planning_p95_ms:.1f}ms (≤{self.planning_p95_threshold}ms) "
            f"{'✅' if results.meets_planning_requirement else '❌'}"
        )
        print(
            f"   Control P95: {results.control_p95_ms:.1f}ms (≤{self.control_p95_threshold}ms) "
            f"{'✅' if results.meets_control_requirement else '❌'}"
        )
        print(
            f"   Actuator P95: {results.actuator_p95_ms:.1f}ms (≤{self.actuator_p95_threshold}ms) "
            f"{'✅' if results.meets_actuator_requirement else '❌'}"
        )
        print(
            f"   Total P95: {results.total_p95_ms:.1f}ms (≤{self.total_p95_threshold}ms) "
            f"{'✅' if results.meets_total_requirement else '❌'}"
        )

        print("\n📊 Overall Status:")
        all_passed = (
            results.meets_planning_requirement
            and results.meets_control_requirement
            and results.meets_actuator_requirement
            and results.meets_total_requirement
        )
...
async def test_real_time_latency_requirements():
    ...
    tester = RealTimeLatencyTester(
        test_duration_seconds=10.0, measurement_frequency_hz=20.0
    )
...
async def test_latency_consistency():
    ...
    tester = RealTimeLatencyTester(
        test_duration_seconds=15.0, measurement_frequency_hz=10.0
    )
    results = await tester.run_latency_test()
...
async def test_extended_latency_stability():
    ...
    tester = RealTimeLatencyTester(
        test_duration_seconds=60.0, measurement_frequency_hz=5.0
    )
    results = await tester.run_latency_test()
    tester.print_results(results)
...
if __name__ == "__main__":
    ...
    async def main():
        tester = RealTimeLatencyTester(
            test_duration_seconds=30.0, measurement_frequency_hz=10.0
        )
        results = await tester.run_latency_test()
        tester.print_results(results)

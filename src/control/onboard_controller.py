...
def _create_pid(gains: PIDGains) -> PIDController:
    return PIDController(gains.Kp, gains.Ki, gains.Kd, integral_limit=gains.integral_limit)
...
class OnboardController:
    ...
    def _interpolate_trajectory(self, current_time: float, trajectory: Trajectory) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        if idx == 0:
            ...
            accel = trajectory.accelerations[0] if trajectory.accelerations is not None else np.zeros(3)
        ...
        if idx >= len(trajectory.timestamps):
            ...
            accel = trajectory.accelerations[-1] if trajectory.accelerations is not None else np.zeros(3)
    ...
    def _compute_desired_attitude_and_thrust(self, desired_accel: np.ndarray, current_yaw: float) -> Tuple[float, float, float]:
        ...
        desired_roll = (1/self.g) * (desired_accel[0] * np.sin(current_yaw) - desired_accel[1] * np.cos(current_yaw))
        desired_pitch = (1/self.g) * (desired_accel[0] * np.cos(current_yaw) + desired_accel[1] * np.sin(current_yaw))
        
        return desired_roll, desired_pitch, thrust

    def _compute_torque(self, desired_roll: float, desired_pitch: float, target_yaw_rate: float, current_state: DroneState, dt: float) -> np.ndarray:
    ...
    def compute_control_command(self, current_state: DroneState, trajectory: Trajectory) -> Tuple[ControlCommand, np.ndarray]:
        ...
        corrective_acceleration = np.array([corrective_accel_x, corrective_accel_y, corrective_accel_z])

        # --- Feedforward + Feedback ---
        desired_accel = target_accel + corrective_acceleration
        
        # --- Attitude & Thrust Command Generation ---
        desired_roll, desired_pitch, thrust = self._compute_desired_attitude_and_thrust(desired_accel, current_state.attitude[2])
        
        # --- Torque Generation (Inner Loop) ---
        target_yaw_rate = 0.0 # Not commanding yaw changes for now
        torque = self._compute_torque(desired_roll, desired_pitch, target_yaw_rate, current_state, dt)

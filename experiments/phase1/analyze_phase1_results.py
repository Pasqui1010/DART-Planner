...
def analyze_phase1_results():
    ...
    try:
        ...
        print(f"✅ Loaded data: {len(df)} records over {df['timestamp'].iloc[-1] - df['timestamp'].iloc[0]:.1f}s")
        ...
        if mean_pos_error < phase1_target:
            ...
            print(f"   ⚠️  NEEDS MORE WORK: {mean_pos_error:.2f}m > {phase1_target}m target")
        ...
        print(f"\n🗺️  ROADMAP PROGRESS:")
        print(f"   Phase 1 (Controller Gains): {'✅ COMPLETE' if mean_pos_error < 30 else '🔄 IN PROGRESS'}")

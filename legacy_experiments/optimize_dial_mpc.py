...
def test_dial_mpc_optimization():
    ...
    if original_analysis and optimized_analysis:
        print(f"\n🎯 Trajectory Quality:")
        print(f"   Final Error: {original_analysis['final_error']:.3f}m → {optimized_analysis['final_error']:.3f}m")
        print(f"   Path Efficiency: {original_analysis['path_efficiency']:.3f} → {optimized_analysis['path_efficiency']:.3f}")
        print(f"   Trajectory Length: {original_analysis['trajectory_length']} → {optimized_analysis['trajectory_length']}")
        
        # Calculate improvements
        error_improvement = ((original_analysis['final_error'] - optimized_analysis['final_error']) 
                           / original_analysis['final_error']) * 100
        efficiency_improvement = ((optimized_analysis['path_efficiency'] - original_analysis['path_efficiency']) 
                                / original_analysis['path_efficiency']) * 100
...
def main():
    ...
    if opt_analysis and orig_analysis:
        error_improvement = ((orig_analysis['final_error'] - opt_analysis['final_error']) 
                           / orig_analysis['final_error']) * 100

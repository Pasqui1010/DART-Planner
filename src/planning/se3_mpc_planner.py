...
class SE3MPCPlanner:
    ...
    def __init__(self, config: Optional[SE3MPCConfig] = None):
        ...
        print(f"  Horizon: {self.config.prediction_horizon} steps ({self.config.prediction_horizon * self.config.dt:.1f}s)")
    ...
    def plan_trajectory(self, current_state: DroneState, goal_position: np.ndarray) -> Trajectory:
        ...
        if self.goal_position is None or np.linalg.norm(self.goal_position - goal_position) > 0.5:
        ...
        try:
            ...
            trajectory = self._create_trajectory_from_solution(solution, current_state.timestamp)
            ...
            if self.plan_count % 10 == 0:
                ...
                success_rate = np.mean(self.convergence_history[-10:]) * 100 if self.convergence_history else 0
                print(f"SE(3) MPC: {avg_time:.1f}ms avg, {success_rate:.0f}% success rate")
    ...
    def _initialize_optimization_variables(self, current_state: DroneState, N: int) -> np.ndarray:
    ...
    def _create_warm_start(self, current_state: DroneState, N: int) -> np.ndarray:
        ...
        if self.goal_position is not None:
            shift_len = min(N-1, len(prev_sol['positions']) - 1) if prev_sol is not None else 0
            for i in range(shift_len+1, N):
                alpha = (i - shift_len) / max(N - shift_len, 1)
                positions[i] = (1-alpha) * positions[shift_len] + alpha * self.goal_position
                thrust_vectors[i] = np.array([0, 0, self.hover_thrust])
        
        return self._pack_variables(positions, velocities, thrust_vectors)
    
    def _create_straight_line_initialization(self, current_state: DroneState, N: int) -> np.ndarray:
        ...
        if self.goal_position is not None:
            ...
            for i in range(N):
                alpha = i / max(N-1, 1)
                positions[i] = (1-alpha) * current_state.position + alpha * self.goal_position
    ...
    def _pack_variables(self, positions: np.ndarray, velocities: np.ndarray, thrust_vectors: np.ndarray) -> np.ndarray:
        """Pack optimization variables into single vector"""
        return np.concatenate([
            positions.flatten(),
            velocities.flatten(), 
            thrust_vectors.flatten()
        ])
    
    def _unpack_variables(self, x: np.ndarray, N: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def _setup_optimization_bounds(self, N: int) -> List[Tuple[float, float]]:
        ...
        for _ in range(N):
            ...
            max_tilt_thrust = self.config.max_thrust * np.sin(self.config.max_tilt_angle)
    ...
    def _setup_optimization_constraints(self, current_state: DroneState, N: int) -> List[Dict]:
        ...
        if self.obstacles:
            constraints.append({
                'type': 'ineq',
                'fun': lambda x: self._obstacle_constraints(x, N)
            })
        
        return constraints
    
    def _dynamics_constraints(self, x: np.ndarray, current_state: DroneState, N: int) -> np.ndarray:
        """
        ...
        for k in range(N-1):
            ...
            acceleration = thrust_vectors[k] / self.mass - np.array([0, 0, self.gravity])
            
            # Position dynamics: p_{k+1} = p_k + v_k*dt + 0.5*a_k*dt^2
            pos_constraint = positions[k+1] - positions[k] - velocities[k]*dt - 0.5*acceleration*dt**2
    ...
    def _dynamics_constraints_jacobian(self, x: np.ndarray, current_state: DroneState, N: int) -> Optional[np.ndarray]:
    ...
    def _create_trajectory_from_solution(self, solution: Dict[str, np.ndarray], start_time: float) -> Trajectory:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': np.mean(self.convergence_history) if self.convergence_history else 0.0,

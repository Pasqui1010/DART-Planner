...
class FastDIALMPCPlanner(DIALMPCPlanner):
    ...
    def _check_trajectory_cache(self, current_state: DroneState, goal_position: np.ndarray) -> Optional[Trajectory]:
        """Check if we have a cached trajectory that's still valid"""
        current_time = time.time()
        
        for cache_key, (cached_trajectory, cache_time, cache_goal) in list(self.trajectory_cache.items()):
            # Check cache age
    ...
    def _shift_cached_trajectory(self, cached_trajectory: Trajectory, current_state: DroneState) -> Trajectory:
    ...
    def _fast_replan(self, current_state: DroneState) -> Trajectory:
        ...
        try:
            if self.goal_position is not None:
                ...
                trajectory = super().plan_trajectory(current_state, current_state.position)
            return trajectory
        finally:
            self.max_iterations = original_iterations
    
    def _incremental_optimization(self, current_state: DroneState, goal_position: np.ndarray) -> Trajectory:
    ...
    def _cache_trajectory(self, current_state: DroneState, goal_position: np.ndarray, trajectory: Trajectory):
        ...
        if len(self.trajectory_cache) > 20:
            # Remove oldest entry
            oldest_key = min(self.trajectory_cache.keys(), 
                           key=lambda k: self.trajectory_cache[k][1])

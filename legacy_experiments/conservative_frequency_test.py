...
class ConservativeFrequencyTest:
    ...
    def reset_to_baseline_gains(self):
        ...
        baseline_gains = {
            'position': {'Kp': np.array([6.0, 6.0, 8.0]), 
                        'Ki': np.array([1.0, 1.0, 2.0]), 
                        'Kd': np.array([4.0, 4.0, 5.0])},
            'attitude': {'Kp': np.array([10.0, 10.0, 4.0]), 
        ...
        print(f"   Position: Kp={baseline_gains['position']['Kp']}, Ki={baseline_gains['position']['Ki']}, Kd={baseline_gains['position']['Kd']}")
        print(f"   Attitude: Kp={baseline_gains['attitude']['Kp']}, Kd={baseline_gains['attitude']['Kd']}")
        print(f"   Feedforward: pos={baseline_gains['feedforward']['pos']}, vel={baseline_gains['feedforward']['vel']}")
    
    def test_frequency_simple(self, target_freq: int, test_duration: float = 10.0) -> dict:
        ...
        instability_rate = large_error_count / cycle_count * 100 if cycle_count > 0 else 100
    ...
    def find_stability_boundary(self):
        ...
        if stable_frequencies:
            ...
            if best_result['mean_velocity_error'] < 5.0:
                ...
                print(f"📊 Velocity improvement: {improvement:.1f}% (vs 9.28m/s baseline)")
            else:
    ...
    def test_optimized_gains_at_stable_frequency(self, stable_freq: int):
        ...
        optimized_gains = {
            'position': {'Kp': np.array([10.0, 10.0, 12.0]), 
                        'Ki': np.array([0.5, 0.5, 1.0]), 
                        'Kd': np.array([6.0, 6.0, 8.0])},
            'attitude': {'Kp': np.array([12.0, 12.0, 5.0]), 

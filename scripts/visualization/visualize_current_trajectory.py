...
def visualize_current_trajectory():
    ...
    try:
        ...
        ax1.plot(df['actual_x'], df['actual_y'], df['actual_z'], 
                'b-', linewidth=2, alpha=0.8, label='Actual Trajectory')
        
        # Plot desired trajectory
        ax1.plot(df['desired_x'], df['desired_y'], df['desired_z'], 
                'r--', linewidth=2, alpha=0.8, label='Desired Trajectory')
        
        # Mark start and end points
        ax1.scatter(df['actual_x'].iloc[0], df['actual_y'].iloc[0], df['actual_z'].iloc[0], 
                   color='green', s=100, label='Start', alpha=0.8)
        ax1.scatter(df['actual_x'].iloc[-1], df['actual_y'].iloc[-1], df['actual_z'].iloc[-1], 
                   color='red', s=100, label='End', alpha=0.8)
        ...
        ax3.text(0.02, 0.98, f'Mean: {mean_error:.2f}m\nMax: {max_error:.2f}m', 
                transform=ax3.transAxes, va='top',
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
        
        # 4. Velocity profile
        ax4 = fig.add_subplot(2, 3, 4)
        actual_speed = np.sqrt(df['actual_vx']**2 + df['actual_vy']**2 + df['actual_vz']**2)
        desired_speed = np.sqrt(df['desired_vx']**2 + df['desired_vy']**2 + df['desired_vz']**2)
...
"""
        ...
        plt.suptitle('Drone Control System - Current Flight Analysis', 

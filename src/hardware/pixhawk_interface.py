...
class PixhawkInterface:
    ...
    async def _planning_loop(self):
        ...
        while self.mission_active and not self.emergency_stop:
            ...
            try:
                ...
                if goal is not None:
                    trajectory = self.planner.plan_trajectory(self.current_state, goal)
        ...
        while self.mission_active:
            if self.performance_stats["control_loop_times"]:
                ...
                if recent_times:
                    ...
                    self.performance_stats["control_frequency_achieved"] = (
                        1000.0 / avg_loop_time
                    )

                # Report performance every 10 seconds
                if len(recent_times) % 100 == 0:
                    avg_planning = np.mean(
                        self.performance_stats["planning_times"][-50:]
                    )
                    print(
                        f"📊 Performance: Control={self.performance_stats['control_frequency_achieved']:.0f}Hz, "
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "control_frequency_achieved": self.performance_stats[
                "control_frequency_achieved"
            ],
...
async def main():
    ...
    if await interface.connect():
        # Simple test mission
        waypoints = np.array(
            [[10.0, 0.0, 5.0], [10.0, 10.0, 5.0], [0.0, 10.0, 5.0], [0.0, 0.0, 5.0]]
        )

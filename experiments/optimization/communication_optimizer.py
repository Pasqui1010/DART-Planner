...
class FastSerializer:
    ...
    def serialize_drone_state(self, state: DroneState) -> bytes:
        ...
        data = struct.pack('d6f', 
                          state.timestamp,
                          float(state.position[0]), float(state.position[1]), float(state.position[2]),
                          float(state.velocity[0]), float(state.velocity[1]), float(state.velocity[2]))
        return data
    ...
    def deserialize_trajectory_compact(self, data: bytes) -> Trajectory:
        ...
        for i in range(n_points):
            ...
            velocities.append(np.array([values[idx+3], values[idx+4], values[idx+5]]))
...
class AsyncCommunicationManager:
    ...
    def __init__(self):
        ...
        print(f"   Trajectory update rate: {1/self.trajectory_receive_interval:.0f}Hz max")
    ...
    def receive_trajectory_async(self) -> Optional[Trajectory]:
        ...
        # Frequency limiting
        if current_time - self.last_trajectory_receive < self.trajectory_receive_interval:
        ...
        while self.running:
            try:
                ...
                if np.random.random() < 0.1:  # 10% chance of receiving trajectory
                    ...
                    data = self.serializer.serialize_trajectory_compact(dummy_trajectory)
...
def run_communication_optimization_test():
    ...
    try:
        ...
        print(f"⏱️  Running {test_duration}s communication test at {control_frequency}Hz...")

...
class ControllerBenchmark:
    ...
    def run_full_benchmark(self) -> Dict[str, Dict[str, BenchmarkResults]]:
        ...
        controllers = {"DIAL-MPC": self.dial_mpc, "SE(3)-MPC": self.se3_mpc}
    ...
    def _run_scenario_test(
        self, planner, controller_name: str, scenario: str
    ) -> BenchmarkResults:
        """Run a single test scenario"""

        # Initialize result object
        result = BenchmarkResults(controller_name=controller_name, scenario=scenario)
        ...
        try:
            while time.time() - test_start_time < self.config.test_duration:
                ...
                ref_pos, ref_vel = self._get_reference_state(
                ...
                if (
                    planning_counter
                    % int(
                        self.config.control_frequency / self.config.planning_frequency
                    )
                    == 0
                ):
                    ...
                    try:
                        current_trajectory = planner.plan_trajectory(
                            current_state, ref_pos
                        )
                ...
                if current_trajectory and len(current_trajectory.positions) > 0:
                    ...
                    desired_vel = (
                        current_trajectory.velocities[0]
                        if current_trajectory.velocities is not None
                        else np.zeros(3)
                    )
                    desired_acc = (
                        current_trajectory.accelerations[0]
                        if current_trajectory.accelerations is not None
                        else np.zeros(3)
                    )
                else:
                ...
                # Simulate system
                current_state = self.simulator.step(
                    current_state, control_output, dt_control
                )
        ...
        if planning_times:
            ...
            result.planning_success_rate = np.mean(
                [t < 100.0 for t in planning_times]
            )  # <100ms = success
    ...
    def _get_reference_state(
        self, reference: Dict[str, Any], time: float
        ...
        if reference["type"] == "static":
            ...
            pos = center + radius * np.array(
                [np.cos(omega * time), np.sin(omega * time), 0.0]
            )
            vel = (
                radius
                * omega
                * np.array([-np.sin(omega * time), np.cos(omega * time), 0.0])
            )
            return pos, vel
            ...
            pos = center + radius * np.array(
                [np.sin(omega * time), np.sin(2 * omega * time) / 2, 0.0]
            )
            vel = (
                radius
                * omega
                * np.array([np.cos(omega * time), np.cos(2 * omega * time), 0.0])
            )
            return pos, vel
    ...
    def _generate_comparison_report(self) -> None:
        ...
        for scenario in self.config.scenarios:
            if scenario in dial_results and scenario in se3_results:
                ...
                print(
                    f"{scenario:20s}: DIAL-MPC {dial_error:.2f}m, SE(3)-MPC {se3_error:.2f}m ({improvement:+.1f}%)"
                )
        ...
        for scenario in self.config.scenarios:
            if scenario in dial_results and scenario in se3_results:
                ...
                print(
                    f"{scenario:20s}: DIAL-MPC {dial_time:.1f}ms, SE(3)-MPC {se3_time:.1f}ms ({speedup:.1f}x speedup)"
                )
        ...
        for scenario in self.config.scenarios:
            if scenario in dial_results and scenario in se3_results:
                ...
                print(
                    f"{scenario:20s}: DIAL-MPC {dial_success:.1f}%, SE(3)-MPC {se3_success:.1f}%"
                )

        # Overall recommendation
        overall_dial_error = np.mean(
            [r.mean_position_error for r in dial_results.values()]
        )
        overall_se3_error = np.mean(
            [r.mean_position_error for r in se3_results.values()]
        )
        overall_improvement = (
            (overall_dial_error - overall_se3_error) / overall_dial_error * 100
        )

        overall_dial_time = np.mean(
            [r.mean_planning_time_ms for r in dial_results.values()]
        )
        overall_se3_time = np.mean(
            [r.mean_planning_time_ms for r in se3_results.values()]
        )
        overall_speedup = (
            overall_dial_time / overall_se3_time
            if overall_se3_time > 0
            else float("inf")
        )
        ...
        print("=" * 60)
        print(
            f"Position Tracking: SE(3)-MPC is {overall_improvement:.1f}% more accurate"
        )
    ...
    def save_detailed_results(
        self, filename: str = "controller_benchmark_results.csv"
    ) -> None:

...
def analyze_phase2b_results():
    ...
    phase2a_file = (
        "improved_trajectory_log_1751106220.csv"  # Phase 2A (failed Kd increase)
    )
    phase2b_file = (
        "improved_trajectory_log_1751106355.csv"  # Phase 2B (feedforward optimization)
    )
    ...
    try:
        ...
        if mean_vel_2b < target_vel_error:
            ...
            print(
                f"   📈 Velocity improvement: {vel_improvement_1_to_2b:.1f}% from Phase 1"
            )
            ...
            print(
                f"   ⚠️  PARTIAL SUCCESS: {mean_vel_2b:.2f}m/s > {target_vel_error}m/s target"
            )
            if vel_improvement_1_to_2b > 0:
                print(
                    f"   📈 Good progress: {vel_improvement_1_to_2b:.1f}% improvement from baseline"
                )
                print(
                    f"   🔧 Recommend: Proceed to Phase 2C (control frequency optimization)"
                )
            elif vel_improvement_2a_to_2b > 0:
                print(
                    f"   📈 Recovery: {vel_improvement_2a_to_2b:.1f}% improvement from 2A failure"
                )
                print(f"   🔧 Recommend: Continue optimization efforts")
            else:
                print(f"   ❌ Further degradation: {vel_improvement_1_to_2b:.1f}%")
                print(
                    f"   🔧 Recommend: Review feedforward settings or try different approach"
                )
        ...
        if mean_vel_2b > mean_vel_1:
            ...
            print(
                f"   🔧 Recommend: Revert to Phase 1 settings and try alternative approach"
            )
        ...
        for i, label in enumerate(percentile_labels):
            change = (
                (vel_percentiles_2b[i] - vel_percentiles_1[i]) / vel_percentiles_1[i]
            ) * 100
            print(
                f"   {label} percentile: {vel_percentiles_1[i]:.2f} → {vel_percentiles_2b[i]:.2f}m/s ({change:+.1f}%)"
            )
        ...
        print(
            f"   Phase 2B (Feedforward): {'✅ COMPLETE' if mean_vel_2b < 5.0 else '❌ FAILED'}"
        )

        if mean_vel_2b > mean_vel_1:
            print(f"   🚨 PROBLEM: Both Phase 2A and 2B made velocity tracking worse")
            print(
                f"   🔧 RECOMMENDATION: Revert to Phase 1 and try control frequency optimization"
            )
        ...
        print(
            f"   • Feedforward ff_vel: 0.8 → 1.5 impact: {vel_improvement_1_to_2b:+.1f}%"
        )
